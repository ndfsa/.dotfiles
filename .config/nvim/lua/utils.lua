...
--- find files function with fallbacks, shows zk plugin topics in a notebook directory, shows
--- git_files in a git repository, fallback to find_files.
M.project_files = function(tl_builtin)
    return function()
        local cwd = vim.fn.getcwd()
        if is_inside_work_tree[cwd] == nil then
            if vim.uv.fs_stat(".zk") then
                is_inside_work_tree[cwd] = "zk"
            else
        ...
        elseif is_inside_work_tree[cwd] == "zk" then
